...
/**
 * @filename:
 * @author: zhangzhijun/@date: 2020/12/1 18:17
 * @version： v1.0
 * @function： 转发
 */
...
public class RepeatActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_repeat_layout);

        if (getIntent() != null){
            mMessage  = (PublicMessage) getIntent().getSerializableExtra("obj_key");

            Log.v("msg" , mMessage.getBody().getType()+"-------");
            Log.v("msg" , mMessage.getBody().getSdkUrl()+"-------");
            Log.v("msg" , mMessage.getBody().getText()+"-------");
//            Log.v("msg" , mMessage.getBody().getImages().get(0).getThumbnailUrl()+"-------");
        }
        initActionBar();
        initView();
        initEvent();
    ...
    private void initView() {
        ...
        checkBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isBoolBan) {
                    ButtonColorChange.checkChange(RepeatActivity.this, checkBox);
                } else {
...
//                new RepeatActivity.UploadTask().execute();
                sendFile();
            }
        });
        ...
        if (mMessage.getBody().getImages() != null && mMessage.getBody().getImages().size() > 0)
            Glide.with(mContext).load(mMessage.getBody().getImages().get(0).getOriginalUrl()).into(mFileIv);
        mFileNameTv.setText(mMessage.getBody().getText());
    }
    ...
    private void initEvent() {
        ...
        mFloatLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                SelectFileDialog dialog = new SelectFileDialog(RepeatActivity.this, new SelectFileDialog.OptionFileListener() {
                    @Override
    ...
    private void isExitNoPublish() {
        if (!TextUtils.isEmpty(mFilePath)) {
            mSelectionFrame = new SelectionFrame(RepeatActivity.this);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == 1 && resultCode == RESULT_OK && data != null) {
            ...
            mFilePath = FileUtils.getPath(RepeatActivity.this, data.getData());
            if (mFilePath == null) {
    ...
    public void sendFile() {
        ...
        PublicMessage.Body body = mMessage.getBody();
        if (body != null){
            if (body.getImages() != null && body.getImages().size() > 0){

                params.put("sdkIcon", body.getImages().get(0).getThumbnailUrl());
                params.put("sdkTitle", body.getTitle());
            }
            String url = coreManager.getConfig().MSG_GET + "?access_token="+ IMUserInfoUtil.getInstance().getToken() + "&messageId="+ mMessage.getMessageId();
        ...
        DialogHelper.showDefaulteMessageProgressDialog(RepeatActivity.this);
        ...
        HttpUtils.post().url(coreManager.getConfig().MSG_ADD_URL)
                ...
                .execute(new BaseCallback<String>(String.class) {
                    ...
                    public void onError(Call call, Exception e) {
                        DialogHelper.dismissProgressDialog();
                        ToastUtil.showErrorNet(RepeatActivity.this);
                    }
                });
    }
    ...
    private class UploadTask extends AsyncTask<Void, Integer, Integer> {
        ...
        protected Integer doInBackground(Void... params) {
            ...
            boolean success = Result.defaultParser(RepeatActivity.this, recordResult, true);
            if (success) {
...
*/
        ...
        protected void onPostExecute(Integer result) {
            ...
            if (result == 1) {
                DialogHelper.dismissProgressDialog();
                startActivity(new Intent(RepeatActivity.this, LoginHistoryActivity.class));
            } else if (result == 2) {
                DialogHelper.dismissProgressDialog();
                ToastUtil.showToast(RepeatActivity.this, getString(R.string.alert_not_have_file));
            } else if (result == 3) {
                DialogHelper.dismissProgressDialog();
                ToastUtil.showToast(RepeatActivity.this, R.string.upload_failed);
            } else {
                sendFile();
            }
        }

...
public class Constants {
    ...
    // 简单密聊
    public final static String SIMPLE_ENCRYPT = "simple_encrypt";
    public final static String SIMPLE_ENCRYPT_CHANGED = "simple_encrypt_changed";
    
    // 火箭消息
    public final static String ROCKET_MESSAGE_CHANGED = "rocket_message_changed";
    
    /**
     * 生成标准化的密聊key，确保发送方和接收方使用相同的key
     * @param userId1 用户ID1
     * @param userId2 用户ID2
     * @return 标准化的密聊key
     */
    public static String generateSimpleEncryptKey(String userId1, String userId2) {
        // 按用户ID排序，确保key的一致性
        if (userId1.compareTo(userId2) < 0) {
            return SIMPLE_ENCRYPT + userId1 + userId2;
        } else {
            return SIMPLE_ENCRYPT + userId2 + userId1;
        }
    }
    
    /**
     * 生成标准化的火箭消息key，确保发送方和接收方使用相同的key
     * @param userId1 用户ID1
     * @param userId2 用户ID2
     * @return 标准化的火箭消息key
     */
    public static String generateRocketMessageKey(String userId1, String userId2) {
        // 按用户ID排序，确保key的一致性
        if (userId1.compareTo(userId2) < 0) {
            return "ROCKET_MESSAGE_" + userId1 + userId2;
        } else {
            return "ROCKET_MESSAGE_" + userId2 + userId1;
        }
    }
    // 聊天背景

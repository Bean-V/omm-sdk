...
import com.oort.weichat.bean.UserStatus;
...
//        }
        ...
        if(user != null) {
            OperLogUtil.msg("lclog" + user.getUserId() + user.getTelephone() + user.getPassword() + user.getNickName());
        }else{

            OperLogUtil.msg("lclog" + "user 为 null");
        }
        return true;
    }
    ...
    public static boolean setLoginUser(Context context, CoreManager coreManager, String telephone, String password, com.xuan.xuanhttplibrary.okhttp.result.ObjectResult<LoginRegisterResult> result) {
        ...
        UserSp sp = UserSp.getInstance(context);
        sp.setAccessToken(result.getData().getAccessToken());
        sp.setHttpKey(result.getData().getHttpKey());
        sp.setUserId(user.getUserId());
        String accessToken = result.getData().getAccessToken();
        //保存token
        spUser.edit().putString("token",accessToken).apply();
        spUser.edit().putString("userid",user.getUserId()).apply();

        UserSp userSp1 = UserSp.getInstance(context);
        userSp1.setUserId(user.getUserId());
        ReportInfo.accessToken = accessToken;
       //获取用户信息及初始化
        User finalUser = user;
        RequesManager.initUserInfoAndData(JSONObject.parseObject(String.valueOf(result)).getString("data"), accessToken, "", new RequesManager.Callback() {
                @Override
                public boolean sucessCallBack() {
                    ...
                    // 保存基本信息到数据库
                    boolean saveAble = UserDao.getInstance().saveUserLogin(finalUser);
                    if (!saveAble) {
                        return false;
                    }
                    ...
                    coreManager.setSelf(finalUser);
                    userSp.setAccessToken(accessToken);

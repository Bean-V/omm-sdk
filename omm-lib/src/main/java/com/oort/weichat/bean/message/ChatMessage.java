package com.oort.weichat.bean.message;
...
import com.oort.weichat.db.dao.ChatMessageDaoImpl;
import com.oort.weichat.helper.MessageSecureHelper;
import com.oort.weichat.util.TimeUtils;

/**
 * @项目名称: 奥陌陌
 * @包名: com.oort.weichat.bean.message
 * @作者:王阳
...
public class ChatMessage extends XmppMessage implements Parcelable {
    public static final Creator<ChatMessage> CREATOR = new Creator<ChatMessage>() {
        ...
        public ChatMessage createFromParcel(Parcel source) {
            ...
            message.toUserName = FastSharedPreferences.get("USERINFO_SAVE").getString("tousername" , "");
            ...
            message.terminal = source.readString();
            // 不参与传输的字段
    ...
    @DatabaseField
    private long fileSize;// 当为图片、语音消息时，此节点有效。图片、语音文件的大小
    @DatabaseField
    private String filePath;// 为语音视频图片文件的 本地路径（IOS端叫fileName），注意本地文件可能清除了，此节点代表的数据不一定有效
    @DatabaseField
    private long timeLen;// 当为语音消息时，此节点有效。语音信息的长度
    @DatabaseField
    ...
    private String toUserName = FastSharedPreferences.get("USERINFO_SAVE").getString("tousername" , "");
    ...
    public JSONObject toJsonObject(@Nullable String messageKey) {
        ...
        object.put("chatType", this.isGroup ? 2:1);
        ...
        object.put("terminal",this.terminal);
        if (messageKey != null) {
    ...
    private void parserJsonData(String jsonData) {
        try {
            ...
            terminal = getStringValueFromJSONObject(jObject,"terminal");
    ...
    /**
     * 检查消息是否为火箭消息（通过terminal字段判断）
     */
    public boolean isRocketMessage() {
        return terminal != null && terminal.equals("ROCKET_MESSAGE");
    }

    /**
     * 设置消息为火箭消息
     */
    public void setRocketMessage() {
        this.terminal = "ROCKET_MESSAGE";
    }

    /**
     * 清除火箭消息标记
     */
    public void clearRocketMessage() {
        if ("ROCKET_MESSAGE".equals(this.terminal)) {
            this.terminal = null;
        }
    }
    ...
    public void setPass(String chatPassWord) {

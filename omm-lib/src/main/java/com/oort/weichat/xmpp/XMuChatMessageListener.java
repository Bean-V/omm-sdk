...
import com.oort.weichat.call.talk.MessageTalkJoinEvent;
import com.oort.weichat.call.talk.MessageTalkLeftEvent;
import com.oort.weichat.call.talk.MessageTalkOnlineEvent;
import com.oort.weichat.call.talk.MessageTalkReleaseEvent;
import com.oort.weichat.call.talk.MessageTalkRequestEvent;
import com.oort.weichat.util.Constants;
import com.oort.weichat.util.DateFormatUtil;
...
public class XMuChatMessageListener implements MessageListener {
    ...
    public void processMessage(Message message) {
        ...
        if (StringUtils.strEquals(message.getStanzaId(), "") || message.getStanzaId() == null) {
             ...
             * @see {@link  NewFriendMessage#toJsonString(boolean)}
             * */
            try {
                JSONObject jsonObject = JSONObject.parseObject(message.getBody());
                message.setStanzaId(jsonObject.getString("messageId"));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        if (delayInformation != null) {// 这是历史记录
            ...
            if (date != null) {
                saveGroupMessage(content, from, message.getStanzaId(), true);
                return;
            }
        }
        saveGroupMessage(content, from, message.getStanzaId(), false);
    }
    ...
    private void saveGroupMessage(String body, String from, String packetId, boolean isDelay) {
        ...
        Log.e("zq", "saveGroupMessage--接收群聊消息");
    ...
    private void chatLive(String body, ChatMessage chatMessage) {
        ...
        int type = chatMessage.getType();
        if (type == XmppMessage.TYPE_SEND_DANMU) {// 弹幕
            Intent intent = new Intent();
            ...
            ListenerManager.getInstance().notifyNewMesssage(mLoginUserId, chatMessage.getObjectId(), chatMessage, true);
        } else if (type == XmppMessage.TYPE_LIVE_EXIT_ROOM) {// 退出、被提出直播间
            Intent intent = new Intent();

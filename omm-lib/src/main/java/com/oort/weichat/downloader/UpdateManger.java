package com.oort.weichat.downloader;
...
public class UpdateManger {
    ...
    private static final String savePath = "/sdcard/updatejwb/";// 保存apk的文件夹
    private static final String saveFileName = savePath + "UpdateJwbRelease.apk";
    ...
    public static void checkUpdate(Activity ctx, String updateUrl, int version,String desc,boolean showtip) {
        if (version == 0 || TextUtils.isEmpty(updateUrl)) {
            // 服务器没有配置新版本，
            return;
        }
        try {
            ...
            manger.checkUpdateInfo(version, updateUrl,desc, showtip);
        } catch (Throwable t) {
            // 无论如何不能因为这个崩溃，
    ...
    // 显示更新程序对话框，供主程序调用
    public void checkUpdateInfo(int serviceVersionCode, String apkUrls,String desc,boolean showtip) {//添加检查服务器更新的代码
...
//        String mCurrentVersionCode = versionName.replaceAll("\\.", "");
        ...
        if (mCurrentVersionCode >= serviceVersionCode) {// 版本号不低 不需要更新
            callNoUpdate();
            if (showtip){
                ToastUtil.showToast(mContext, "您已是最新版本");
            }
        } else {
            apkUrl = apkUrls;
            showNoticeDialog(desc, getStringById(R.string.application_update));
        }
    }

package com.oort.weichat.ui.message.single;
...
import com.oort.weichat.pay.TransferRecordActivity;
...
import com.oort.weichat.view.SelectChatModeDialog;
...
public class PersonSettingActivity extends BaseActivity implements View.OnClickListener {
    ...
    private SwitchButton mRocketMessage;
    ...
    private SwitchButton mSimpleEncryptSb; // 简单密聊开关
    ...
    private void initView() {
        ...
        // 简单密聊开关
        mSimpleEncryptSb = (SwitchButton) findViewById(R.id.sb_simple_encrypt);

        boolean isSimpleEncryptEnabled = PreferenceUtils.getBoolean(mContext, Constants.generateSimpleEncryptKey(mFriendId, mLoginUserId), false);
        mSimpleEncryptSb.setChecked(isSimpleEncryptEnabled);
        mSimpleEncryptSb.setOnCheckedChangeListener((view, isChecked) -> {
            updateSimpleEncryptStatus(isChecked);
        });

        //火箭消息
        mRocketMessage = (SwitchButton) findViewById(R.id.rocket_message);
        boolean isRocketEnabled = PreferenceUtils.getBoolean(mContext, Constants.generateRocketMessageKey(mFriendId, mLoginUserId), false);
...
//                                FriendDao.getInstance().updateFriendPartStatus(mFriend.getUserId(), user);
                                runOnUiThread(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        mIsCopySb = (SwitchButton) findViewById(R.id.sb_ban_copy);
                                        mIsCopySb.setChecked(user.getFriends().getCanCopy() == 0);// TopTime不为0，当前状态为置顶
                                        mIsCollectbSb = (SwitchButton) findViewById(R.id.sb_ban_colect);
                                        mIsCollectbSb.setChecked(user.getFriends().getCanCollect() == 0);// TopTime不为0，当前状态为置顶
                                        mIsShotSb = (SwitchButton) findViewById(R.id.sb_ban_capture);
                                        mIsShotSb.setChecked(user.getFriends().getScreenshotProhibited() == 0);// TopTime不为0，当前状态为置顶
                                        mIsTransbSb = (SwitchButton) findViewById(R.id.sb_ban_trans);
                                        mIsTransbSb.setChecked(user.getFriends().getCanForward() == 0);
...
//        }
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.iv_title_left) {
            finish();
        } else if (id == R.id.avatar) {//                Intent intentBasic = new Intent(this, BasicInfoActivity.class);
//                intentBasic.putExtra(AppConstant.EXTRA_USER_ID, mFriendId);
//                startActivity(intentBasic);


            PersonDetailActivity.actionStart(this, mFriendId);
        } else if (id == R.id.add_contacts) {//                Intent intentAdd = new Intent(this, SelectContactsActivity.class);
...
//                startActivity(intentAdd);
            ...
            arrayList.add(mFriendId);

            tool.ceateRoom(this, coreManager, arrayList);
    ...
    private void updateChatSet(int type, boolean isChecked, SwitchButton sb) {
        ...
        HttpUtils.get().url(com.oortcloud.basemodule.constant.Constant.IM_API_BASE + "/friends/update")
                .params(params)
                .build()
                .execute(new BaseCallback<Void>(Void.class) {
    ...
    private void updateSimpleEncryptStatus(boolean isEnabled) {
        // 保存到本地偏好设置
        PreferenceUtils.putBoolean(mContext, Constants.generateSimpleEncryptKey(mFriendId, mLoginUserId), isEnabled);

        if (isEnabled) {
            ToastUtil.showToast(mContext, "密聊已开启，消息将自动加密");
        } else {
            ToastUtil.showToast(mContext, "密聊已关闭");
        }
    ...
    private void updateRocketMessageStatus(boolean isEnabled) {
        // 保存到本地偏好设置
        PreferenceUtils.putBoolean(mContext, Constants.generateRocketMessageKey(mFriendId, mLoginUserId), isEnabled);
        
        if (isEnabled) {
            ToastUtil.showToast(mContext, "火箭消息已开启");
        } else {
            ToastUtil.showToast(mContext, "火箭消息已关闭");
        }
                ...
                .execute(new BaseCallback<Void>(Void.class) {
                    ...
                    public void onResponse(ObjectResult<Void> result) {
                        ...
                        if (result.getResultCode() == 1) {
                            ...
                            sendBroadcast(new Intent(OtherBroadcast.NAME_CHANGE));// 刷新聊天界面
                        } else {

...
import com.oort.weichat.ui.live.adapter.MemberAdapter;
import com.oort.weichat.ui.live.bean.Detail;
import com.oort.weichat.ui.live.bean.Gift;
import com.oort.weichat.ui.live.bean.GiftItem;
import com.oort.weichat.ui.live.bean.Member;
import com.oort.weichat.ui.live.view.FragmentDialog;
import com.oort.weichat.ui.live.view.FragmentEditDialog;
import com.oort.weichat.ui.live.view.FragmentPayDialog;
import com.oort.weichat.ui.live.view.GiftItemView;
import com.oort.weichat.ui.live.view.HorizontialListView;
import com.oort.weichat.ui.me.redpacket.WxPayBlance;
import com.oort.weichat.util.PreferenceUtils;
import com.oort.weichat.util.TimeUtils;
import com.oort.weichat.util.ToastUtil;
import com.oort.weichat.view.BannedDialog;
import com.oort.weichat.view.ChatContentView;
import com.oort.weichat.view.ManagerPopupWindow;
import com.oort.weichat.view.PullDownListView;
import com.oort.weichat.xmpp.ListenerManager;
import com.oort.weichat.xmpp.ReceiptManager;
    ...
    private BroadcastReceiver changeSomeBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(LiveConstants.LIVE_MEMBER_ADD)) {
                ...
                getActivity().finish();
            }else if (action.equals(LiveConstants.LIVE_SWITCHTO_SCREEN)) {
//                mSrsPublisher.stopPublish();
            }
            else if (action.equals(LiveConstants.LIVE_SWTICHTO_LIVEROOM)) {

                String url = sharedPreferences.getString("share_screen","");
                mSrsPublisher.startPublish(url);
                mSrsPublisher.switchCameraFace((mSrsPublisher.getCamraId()) % Camera.getNumberOfCameras());
            }
        }
    };
    ...
    /**
     * todo: 这个fragment没有无参构造方法，直播中后台activity被销毁回来会导致崩溃，java.lang.RuntimeException: Unable to start activity ComponentInfo{com.btchat.weichat/com.oort.weichat.ui.live.PushFlowActivity}: android.support.v4.app.Fragment$InstantiationException: Unable to instantiate fragment com.oort.weichat.ui.live.AnchorChatFragment: could not find Fragment constructor
     *
     * @param SrsPublisher
     * @param roomId
     * @param roomJid
    ...
    protected void onActivityCreated(Bundle savedInstanceState, boolean createView) {
        if (createView) {
            ...
            filter.addAction(LiveConstants.LIVE_SWITCHTO_SCREEN);
            filter.addAction(LiveConstants.LIVE_SWTICHTO_LIVEROOM);
    ...
    private void initView() {
        ...
        mDanMuS.setVisibility(View.GONE);
        ...
        // 切换屏幕共享
        findViewById(R.id.share_screen).setOnClickListener(this);
        // 切换摄像头
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.share_screen) {
            ...
            切换屏幕共享
             */
            Intent mIntent = new Intent("net.yrom.screenpush.ScreenRecord.xxxxxx" );
            String url = sharedPreferences.getString("share_screen","");
            mIntent.putExtra("url",url);
            mIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);

            startActivity(mIntent);
//            Toast.makeText(getActivity(),"切换到屏幕共享",Toast.LENGTH_LONG).show();
        }
    }

...
public class AuthCodeActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        } else if (id == R.id.switch_account_btn) {
            finish();
    ...
    private void requestImageCode() {
        ...
        String url = HttpUtils.get().url(coreManager.getConfig().USER_GETCODE_IMAGE)
                .params(params)
                .buildUrl();
        ImageLoadHelper.loadBitmapCodeAnimate(mContext, url, new ImageLoadHelper.BitmapSuccessCallback() {
            @Override
            public void onSuccess(Bitmap b) {
                mImageCodeIv.setImageBitmap(b);
            }
        }, new ImageLoadHelper.ImageFailedCallback() {
            @Override

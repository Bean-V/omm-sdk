package com.oort.weichat.audio_x;

import static com.oort.weichat.audio_x.VoiceManager.STATE_PLAY;
...
public class VoiceAnimView extends RelativeLayout {
    ...
    private void bindData(boolean direction, String id,
        ...
        mSeekBar.setMax(Math.toIntExact(length));
        mSeekBar.setVisibility(GONE);

        LayoutParams params = (LayoutParams) mFlSeek.getLayoutParams();
        params.width = DisplayUtil.getVoiceViewWidth(mContext, Math.toIntExact(length));
    ...
    public void fillData(PublicMessage message) {
        bindData(
                ...
                message.getBody().getAudios().get(0).getLength(),
                true
        );
    }

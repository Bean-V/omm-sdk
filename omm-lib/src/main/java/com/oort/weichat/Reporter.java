...
import com.oort.weichat.util.CrashHandler;
...
//        Bugly.init(ctx, BuildConfig.BUGLY_APP_ID, BuildConfig.DEBUG && Log.isLoggable("Bugly", Log.DEBUG));
...
//        String androidId = android.provider.Settings.Secure.getString(ctx.getContentResolver(), android.provider.Settings.Secure.ANDROID_ID);
        String androidId = CommonApplication.GUID;
        // 登录前是设备唯一码，登录后是手机号，
        CrashReport.setUserId(androidId);
        CrashReport.setAppChannel(ctx, BuildConfig.BUGLY_APP_CHANNEL);
//        CrashReport.setAppVersion(ctx, BuildConfig.LIBRARY_PACKAGE_NAME + BuildConfig.VERSION_NAME_SUFFIX);
    }
    ...
    public static void putUserData(String key, String value) {
        ...
        CrashHandler.getInstance().putCustomInfo(key, value);
    }

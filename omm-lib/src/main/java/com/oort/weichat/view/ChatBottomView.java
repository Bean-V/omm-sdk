...
import com.oort.weichat.audio.IMRecordController;
import com.oort.weichat.audio.RecordListener;
...
import com.oort.weichat.db.InternationalizationHelper;
...
import com.oort.weichat.util.InputManager;
...
import com.oort.weichat.util.input.KeyboardPanelUtil;
...
public class ChatBottomView extends LinearLayout implements View.OnClickListener {
    ...
    OnClickListener publicMenuClickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            int id = v.getId();
            if (id == R.id.ll_public_meun_1) {
                publicMenuClick(0);
            } else if (id == R.id.ll_public_meun_2) {
                publicMenuClick(1);
            } else if (id == R.id.ll_public_meun_3) {
                publicMenuClick(2);
            }
        }
    };
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.send_btn) {// 发送文字与戳一戳，不受过快点击限制影响，
            ...
            if (id == R.id.voice_img_btn) {
            ...
            } else if (id == R.id.btnCancelReplay) {
                resetReplay();
                ...
                changeRecordBtn(false);
                inputText();
            } else if (id == R.id.chat_edit) {// 隐藏其他所有布局，显示键盘
                if (isBottomViewShowing()) {
                    postHideBottomView();
                }
                changeRecordBtn(false);
            ...
            } else if (id == R.id.more_btn) {// 点击加号取消回复，
                if (replayMode) {
                    resetReplay();
                    mBottomListener.cancelReplay();
                }
...
//                        showKeyboard();
    ...
    private void showBottomView() {
        ...
        // 最小高度，
        int minHeight = DisplayUtil.dip2px(getContext(), 300);
        if (height < minHeight) {
            height = minHeight;
        }
    ...
    public void switchToSecretChatMode(boolean b) {
        mChatToolsView.switchToSecretChatMode(b);
    }

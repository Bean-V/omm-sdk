...
public class PendingRequests {
     ...
     */
    public synchronized Request createRequest(int action, JSONArray options, CallbackContext callbackContext) throws JSONException {
    ...
    public class Request {
        ...
        private Request(int action, JSONArray options, CallbackContext callbackContext) throws JSONException {
            this.callbackContext = callbackContext;
            this.action = action;

            if (options != null) {
                if (options.length() > 2) {
                    LOG.w(LOG_TAG, "Expected at most 3 arguments to capture() but got " + options.length());
                    this.limit = options.getInt(0);
                    this.duration = options.getInt(1);
                    this.quality = 1;//options.getInt(2);
                }

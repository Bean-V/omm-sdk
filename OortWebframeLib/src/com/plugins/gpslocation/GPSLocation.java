/*
       ...
       under the License.
 */
package com.plugins.gpslocation;
...
public class GPSLocation extends CordovaPlugin {
	...
	public boolean execute(final String action, final JSONArray args,
		...
		if (action == null || !action.matches("getLocation|addWatch|clearWatch|getAddress")) {
			return false;
		}
		...
		if (action.equals("getLocation")) {
			...
			BigDecimal bd = new BigDecimal(latitude).setScale(8, RoundingMode.HALF_UP);
			double roundedLatitude = bd.doubleValue();

			double longitude = Double.parseDouble(sharedPreferences.getString("longitude", "0"));
			BigDecimal ld = new BigDecimal(longitude).setScale(8, RoundingMode.HALF_UP);
			double roundedLongitude = ld.doubleValue();
			String address = sharedPreferences.getString("address","");
			if (latitude == 0.0 || longitude == 0.0  || address.isEmpty() || address == ""){
				callbackContext.error("get address failure");
				return true;
			}
			JSONObject o = new JSONObject();

			try {
				o.put("latitude", roundedLatitude);
				o.put("longitude", roundedLongitude);

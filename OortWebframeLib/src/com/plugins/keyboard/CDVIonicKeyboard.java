...
public class CDVIonicKeyboard extends CordovaPlugin {
    ...
    public void onDestroy() {
        // 1. 先判断 rootView 是否为 null
        if (rootView != null) {
            ViewTreeObserver observer = rootView.getViewTreeObserver();
            // 2. 判断 ViewTreeObserver 是否为 null 且是否存活（避免已失效的 observer）
            if (observer != null && observer.isAlive()) {
                // 3. 判断监听对象 list 是否为 null
                if (list != null) {
                    // 4. 兼容 Android API 16+，使用更安全的移除方法
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                        observer.removeOnGlobalLayoutListener(list);
                    } else {
                        // 低版本兼容（API < 16）
                        observer.removeGlobalOnLayoutListener(list);
                    }
                }
            }
        }

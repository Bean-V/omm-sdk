package com.oortcloud.basemodule.views.leafchart;
...
import com.oortcloud.basemodule.views.leafchart.bean.ChartData;
import com.oortcloud.basemodule.views.leafchart.bean.PointValue;
import com.oortcloud.basemodule.views.leafchart.renderer.AbsRenderer;
import com.oortcloud.basemodule.views.leafchart.support.LeafUtil;
import com.oortcloud.basemodule.views.leafchart.support.Mode;
 ...
 * </br>
 */
public abstract class AbsLeafChart extends View implements com.oortcloud.basemodule.views.leafchart.support.Chart {

    static final String TAG = com.oortcloud.basemodule.views.leafchart.SlideSelectLineChart.class.getName();
    ...
    protected void resetAsixY() {
        if (axisY != null) {
            ...
            if(axisX != null) {
                int sizeX = axisX.getValues().size();
                for (int i = 0; i < sizeX; i++) {
                    AxisValue axisValue = axisX.getValues().get(i);
                    if (count < axisValue.getLabel().length()) {
                        count = axisValue.getLabel().length();
                    }
                }
            }else{
                count = 5;
            ...
            float yStep = (mHeight - topPadding - bottomPadding - startMarginY - count * 15) / sizeY;

            for (int i = 0; i < sizeY; i++) {
                AxisValue axisValue = values.get(i);
                axisValue.setPointX(leftPadding);
                if (i == 0) {
                    axisValue.setPointY(mHeight - bottomPadding - startMarginY - count * 15);
                } else {
                    axisValue.setPointY(mHeight - bottomPadding - startMarginY - count * 15  - yStep * i);
                }
            }
            switch (coordinateMode) {
                case Mode.ACROSS:
                case Mode.Y_ACROSS:
                    axisY.setStartY(mHeight - bottomPadding * 0.5f - count * 15);
                    break;

                case Mode.INTERSECT:
                case Mode.X_ACROSS:
                    axisY.setStartY(mHeight - bottomPadding - count * 15);
                    break;
            }
    ...
    protected void resetAsixX() {
        if (axisX != null) {
            ...
            for (int i = 0; i < sizeX; i++) {
                AxisValue axisValue = values.get(i);
                axisValue.setPointY(mHeight - count * 15);
                if (i == 0) {
            ...
            axisX.setStartY(mHeight - bottomPadding - count * 15).setStopX(mWidth).setStopY(mHeight - bottomPadding-count * 15);
        }
    }

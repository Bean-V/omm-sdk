            ...
            if (axisX.isShowText()) {
                for (int i = 0; i < valuesX.size(); i++) {
                    ...
                    if (value.isShowLabel()) {
                        ...
                        //canvas.rotate(-45,textW / 2,textH / 2);

                        Path p = new Path();
                        p.moveTo(value.getPointX(),value.getPointY()+textW);

                        p.lineTo(value.getPointX(),value.getPointY());

                        canvas.drawPath(p,coordPaint);
                        canvas.drawTextOnPath(value.getLabel(),p,0,0,coordPaint);
                    }
                }
            }
        ...
        if (chartData != null) {
            if (chartData.isHasLabels()) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    labelPaint.setColor(Color.parseColor("#ffffff"));//chartData.getLabelColor()

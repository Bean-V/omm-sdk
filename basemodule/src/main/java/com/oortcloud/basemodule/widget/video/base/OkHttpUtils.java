package com.oortcloud.basemodule.widget.video.base;
                    ...
                    if(REQUST_OK==response.code()){
                        try {
                            ...
                            if(!TextUtils.isEmpty(string)){
                                if(DEBUG) Log.d(TAG,"服务端返回数据-->"+string);
...
//        }
        try {
            if(null!=type){
                resultData= JSON.parseObject(json, type);
            }else{
                //如果用户没有指定Type,则直接使用String.class
                resultData=JSON.parseObject(json, new TypeToken<String>(){}.getType());
            }
            return resultData;
    ...
    private void download(final String path, String outPutPath, String outPutFileName, final OnDownloadListener listener) {
        ...
        createHttpUtils().newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(okhttp3.Call call, final IOException e) {
                if(DEBUG){
                    Log.d(TAG,"onFailure-->e:"+e.getMessage()+call.toString());
                }
                isRequst=false;
                    ...
                    if(null!=response){
                        if(200==response.code()){
                            try {
                                ...
                                Log.d(TAG,"目标存储路径："+apkDownloadPath.getAbsolutePath());

...
import com.oortcloud.appstore.utils.AppUtils;
...
public class AppInfo extends DataSupport implements Serializable {
    ...
    public AppInfo(String app_id, String applabel){
        this.app_id = app_id;
    ...
    // 状态 0:不可用 1:可使用(配置可直接用或者已审核通过) 2:可申请 3:已申请 4:已驳回
    ...
    public boolean isHas_update() {
        ...
        if(getTerminal() == 0){
            int code = AppUtils.getVersionCode(AppStoreInit.getInstance().getApplication(), getApppackage());
            String version = AppUtils.getVersionName(AppStoreInit.getInstance().getApplication(),getApppackage());
            if(code < getNew_versioncode()){
                have_update = true;
            }else{
                have_update = false;
            }
        }else if(getTerminal() == 1){
            if(!AppInfoManager.getInstance().isContains(this) || !AppManager.checkFilePath(this)){
                have_update =  true;
            }else{
                have_update = false;
            }
    ...
    public String getAppweburl() {
        if (!TextUtils.isEmpty(appweburl)){
            //平台上部署的web应用才做替换，否则不用做替换
            int index = appweburl.indexOf("oort/");
            if (index != -1) {
                appweburl = HttpConstants.BASE_URL + appweburl.substring(index);
                return appweburl;
            }
        }
        return appweburl;
    ...
    public String getApk_url() {
        ...
        if (!TextUtils.isEmpty(apk_url) && apk_url.contains("oort/")){
            apk_url =  HttpConstants.BASE_URL +apk_url.substring(apk_url.indexOf("oort/") );
            return apk_url;
    ...
    public String getIcon_url() {
        if (!TextUtils.isEmpty(icon_url)&& icon_url.contains("oort/")){
            icon_url =  HttpConstants.BASE_URL +icon_url.substring(icon_url.indexOf("oort/") );
            return icon_url;
        }
        return icon_url;
    }

...
public class RetrofitServiceManager {
    ...
    private RetrofitServiceManager(String baseurl) {
        ...
        // 使用 map 操作符对响应数据进行转换
        // 这里可以统一处理响应数据，例如解析公共字段
//        apiService.getData()
//                .map(HttpAPI.DataResponse::getMessage)
//                .doOnSuccess(data -> {
//                    // 使用 doOnSuccess 操作符进行日志记录
//                    System.out.println("Received data: " + data);
//                })
//                .onErrorResumeNext(throwable -> {
//                    // 使用 onErrorResumeNext 操作符处理错误
//                    System.err.println("Error: " + throwable.getMessage());
//                    return Single.just("Error occurred");
//                })
//                .subscribe(
...
//                );
    ...
    static Observable<String> PostObservable_url(String url, HashMap<String, Object> headerMap, HashMap<String, Object> bodyMap) {
        ...
        OperLogUtil.msg(url);
        return apiService_.PostAppListbservable(url, requestBody ,headerMap ).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread());
    }
